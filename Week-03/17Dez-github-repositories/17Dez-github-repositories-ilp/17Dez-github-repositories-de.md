...
- Es wird wie ein Verzeichnis auf den Servern von GitHub gespeichert.
- Repositories können entweder `öffentlich` (jeder kann sie sehen) oder `privat` (nur du und von dir ausgewählte Personen können sie sehen) sein.
...
### Sichtbarkeitseinstellungen für Repositories

- Beim Erstellen eines Repositorys auf GitHub hast du die Möglichkeit, dessen Sichtbarkeit auf `öffentlich` oder `privat` einzustellen.
- Diese Optionen erscheinen in VS Code, nachdem du auf `Publish to GitHub` geklickt hast.
- Du kannst die Sichtbarkeitseinstellungen auch später auf der GitHub-Website ändern.
  - **Öffentliches Repository**:
    - Jeder kann das Repository und dessen Inhalt sehen.
    - Ideal für Projekte, die du mit potenziellen Arbeitgebern teilen oder auf deinem GitHub-Profil anpinnen möchtest.
  - **Privates Repository**:
    - Nur du und andere GitHub-Benutzer, denen du explizit Zugriff gewährst, können das Repository sehen.
    - Geeignet für persönliche Projekte oder Arbeiten, die du nicht öffentlich teilen möchtest, zum Beispiel deine täglichen CTF-Notizen.
...
- Hier sind einige grundlegende Git-Befehle im Zusammenhang mit der Arbeit mit lokalen Repositories:
  ...
  - `git push -u origin main`: Pusht die committeten Änderungen in das Remote-Repository auf GitHub (nur beim ersten Push, danach kannst du `git push` verwenden).
...
- Lass uns ein Profil-README für dein GitHub-Profil erstellen, indem wir die besprochenen Schritte befolgen.
- Lass uns versuchen, eine `.gitignore`-Datei zu erstellen, um alle sensiblen Dateien zu ignorieren, die du möglicherweise in deinem Projektverzeichnis hast.
